...
-- ================================================================
-- Apply this as the primary schema after moving login to Supabase Auth.
-- The old browser-password model is archived in docs/legacy/schema-legacy.sql.
--
-- Important:
...
-- ---------- Legacy cleanup ----------
-- If you are migrating from the old prototype schema, keep the old tables from
-- being reachable by the browser while the app moves to Supabase Auth.
do $$
begin
  if to_regclass('public.users') is not null then
    execute 'alter table public.users enable row level security';
    execute 'drop policy if exists "allow_all_users" on public.users';
    execute 'drop policy if exists "allow_all_users_dev" on public.users';
  ...
  if to_regclass('public.submissions') is not null then
    ...
    execute 'drop policy if exists "allow_all_submissions" on public.submissions';
    execute 'drop policy if exists "allow_all_submissions_dev" on public.submissions';
    execute 'revoke all on public.submissions from anon, authenticated';
  end if;

  if to_regclass('public.tests') is not null then
    execute 'drop policy if exists "allow_all_tests" on public.tests';
    execute 'drop policy if exists "allow_all_tests_dev" on public.tests';
  end if;

  if to_regclass('public.questions') is not null then
    execute 'drop policy if exists "allow_all_questions" on public.questions';
    execute 'drop policy if exists "allow_all_questions_dev" on public.questions';
  end if;
end $$;
...
alter table public.tests
  ...
  add column if not exists rw_minutes int not null default 64,
  add column if not exists math_minutes int not null default 70,
...
alter table public.questions
  add column if not exists module_key text not null default 'rw1',
...
-- Leaderboard-safe views. These expose only profile display data and submitted
-- scores, never raw answer JSON.
...
create or replace view public.leaderboard_attempts
as
select
  id,
  student_id,
...
create unique index if not exists idx_attempts_one_submitted_per_test
...
create policy "attempts_student_select" on public.test_attempts
  for select using (student_id = auth.uid());
create policy "attempts_student_insert" on public.test_attempts
  for insert with check (student_id = auth.uid() and status = 'in_progress');
...
-- Students read only through public.student_questions. Admins read full question
-- content through public.admin_questions, which checks public.is_admin().
...
grant select on public.leaderboard_attempts to authenticated;
